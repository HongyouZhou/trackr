...
def main(config: DictConfig):
    ...
    if config.pretrain.wandb_activate:
        wandb.init(
            project="manipulation-pretraining",
    ...
    if config.pretrain.test:
        ...
        cprint(
            f"Model loaded from {config.pretrain.checkpoint}",
            color="green",
            attrs=["bold"],
        )
        ...
        model = HumanModel(cfg=config, max_ep_len=max_ep_len)
        ...
        if config.pretrain.training.model_save_dir is not None:
            ...
            experiment_folder = os.path.join(
                save_dir, f"{config.pretrain.wandb_name}", f"dt_{dt_string}"
            )
        ...
        for i in range(config.pretrain.training.num_epochs):
            ...
            outputs = trainer.train_epoch(iter_num=i, print_logs=True)
            if config.pretrain.wandb_activate:
            ...
            if capture_video:
                ...
                logger.log_video(
                    ...
                    fps=fps,
                    step=(i + 1) * len(trainer.train_dataloader),
                )
                logger.log_dict(
                    info_dict, (i + 1) * len(trainer.train_dataloader), verbose=False
                )
                env.video_frames = []
